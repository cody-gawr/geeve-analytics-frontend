...
export class AuthFacade {
  ...
  public readonly isLoading$: Observable<boolean> = this.store.pipe(select(selectIsLoading));
  ...
  public readonly chartLimitDesc$: Observable<string> = this.store.pipe(
    ...
    map(
      v =>
        `Results plotted on the graph are limited to ${v.maxChartBars} due to your settings. This can be updated via Settings -> My Account -> Customisations -> Max Chart Bars. Totals beneath the chart still reflect the full amount.`,
    ),
  );
  ...
  public readonly rolesIndividual$: Observable<RolesIndividualApiResponse | null> = this.store.pipe(
    select(selectRolesIndividual),
  );

  public readonly isClinicianUser$ = this.store.pipe(select(selectIsClinicianUser));
