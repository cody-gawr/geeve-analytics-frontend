...
@Component({
    selector: 'fd-reappoint-rate-chart',
    templateUrl: './reappoint-rate.component.html',
    styleUrls: ['./reappoint-rate.component.scss']
})
export class FrontDeskReappointRateComponent implements OnInit, OnDestroy {
    @Input() toolTip = '';
    ...
    get showGoals$() {
        return this.layoutFacade.dateRange$.pipe(
            ...
            map(val => {
                ...
                if(val.start.date() == 1 && val.end.date() == val.end.clone().endOf('month').date()){
                    return true;
                }
        ...
        return combineLatest([
          ...
          map(([isTrend, isMulti]) => {
            if(isTrend || isMulti){
              ...
              return this.fdReappointRateVal > 0;
            }
          })
        )
      }
    ...
    constructor(
        ...
        combineLatest([
            ...
            this.frontDeskFacade.fdReappointRateTrendChartData$,
        ]).pipe(
            takeUntil(this.destroy$)
        ).subscribe(([
            isTrend, chartData, trendChartData
                ...
                if(isTrend){
                    ...
                    this.fdReappointRateVal = chartData.fdReappointRateVal;
                    this.fdReappointRatePrev = chartData.fdReappointRatePrev;
                    this.fdReappointRateGoal = chartData.fdReappointRateGoal;
                }
        });
    }
