...
import { externalTooltipHandler, externalTooltipHandlerHiddenColorBoxes, generatingLegend_3, JeeveLineFillOptions } from "@/newapp/shared/utils";
...
export class FnHourlyRateChartComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        this.financeFacade.hourlyRateChartData$.pipe(
    ...
    get chartType$() {
        return combineLatest([
            ...
            map(([trend, isMulti]) => trend === 'off' ? (isMulti? 'bar': 'arch') : 'line')
        )
    }
        ...
        return combineLatest([
          ...
          map(([isMulti, trend]) => {
            if (trend !== 'off' || isMulti) {
              return this.labels.length > 0 && this.datasets.length > 0 && this.datasets[0].data.length > 0;
            } else {
              return this.curr > 0;
            }
          })
    ...
    get durationCurrLabel$() {
        return combineLatest([
            this.layoutFacade.trend$,
            this.layoutFacade.durationCurrLabel$
        ]).pipe(map(([trend, curLabel]) => trend === 'off'? curLabel: null ))
    }

    get durationPrevLabel$() {
        return combineLatest([
            this.layoutFacade.trend$,
            this.layoutFacade.durationPrevLabel$
        ]).pipe(map(([trend, prevLabel]) => trend === 'off'? prevLabel: null ))
    }
