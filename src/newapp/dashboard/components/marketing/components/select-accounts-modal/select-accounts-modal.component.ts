  ...
  export class MkSelectAccountsModalComponent implements OnInit, OnDestroy {
    ...
    onSubmitClick() {
        ...
        .pipe(takeUntil(
            ...
            if(isCon == 'xero'){
                this.marketingFacade.saveXeroAccounts({clinicId, categories: this.selectedAccounts});
            }else if(isCon == 'myob'){
                this.marketingFacade.saveMyobAccounts({clinicId, categories: this.selectedAccounts});
            }
        })
    }
  }