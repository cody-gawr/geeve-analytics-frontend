...
import { 
    FdFtaRatioApiResponse, FdFtaRatioTrendApiResponse, 
    FdNumTicksApiResponse, FdNumTicksTrendApiResponse, 
    FdReappointRateApiResponse, FdReappointRateTrendApiResponse, 
    FdRecallRateApiResponse, FdRecallRateTrendApiResponse, 
    FdUtaRatioApiResponse, FdUtaRatioTrendApiResponse, 
    FdUtilisationRateApiResponse, FdUtilisationRateByDayApiResponse, 
...
type FrontDeskEndpoints =  
    'fdUtilisationRate' | 'fdUtilisationRateTrend' | 
    'fdUtilisationRateByDay' | 
    'fdRecallRate' | 'fdRecallRateTrend' |
    'fdReappointRate' | 'fdReappointRateTrend' | 
    'fdNumTicks' | 'fdNumTicksTrend' | 
    'fdFtaRatio' | 'fdFtaRatioTrend' |
    'fdUtaRatio' | 'fdUtaRatioTrend';
...
export const selectFdUtilRateChartData = createSelector(
    ...
    (resBody) => {
        ...
        const fdUtiData = [], chartData = [], chartLabels = [];
...
export const selectFdUtilRateByDayChartData = createSelector(
    ...
    (resBody) => {
        ...
        const chartData = [], chartLabels = [], fdUtiByDayData = [];
        ...
        .map(
            ...
            item => {
                ...
                chartLabels.push(`${item.day}--${item.workedHour}--${item.plannedHour}`)
            }
        )
    ...
    (resBody, clinicId, trendMode) => {
        ...
        if(typeof clinicId === 'string'){
            ...
            resBody.data.forEach(
                item => {
                    chartData.push(
                        Math.round(parseFloat(<string>item.utilRate) * 100)
                    );
                    if(item.goals == -1 || item.goals == '' || item.goals == null){
                        targetData.push([0, 0]);
                    } else {
                        targetData.push([parseFloat(<string>item.goals) - 0.5, parseFloat(<string>item.goals) + 0.5]);
                    }
                    chartLabels.push(
                        `${trendMode == 'current'?moment(item.yearMonth).format('MMM YYYY'):item.year}--${item.workedHour}--${item.plannedHour}`);
                }
            );
            chartDatasets = [
                {
        ...
        if(typeof clinicId === 'string'){
            ...
            resBody.data.forEach(
                item => {
                    if(item.clinicId){
                        chartData.push(
                            Math.round((parseFloat(<string>item.recallPatient) / parseFloat(<string>item.totalPatient)) * 100)
                        );
...
export const selectFdRecallRateTrendChartData = createSelector(
    ...
    (resBody, clinicId, trendMode) => {
        ...
        if(typeof clinicId === 'string'){
            ...
            chartDatasets = [{data: chartData, backgroundColor: DoughnutChartColors[0], label: ''}];
        }else{
            const chartData = [], targetData = [];
            resBody.data.forEach(
                item => {
    ...
    (resBody) => {
        ...
        if(resBody.total > 0){
            resBody.data.forEach(
                item => {
                    chartData.push(
                        Math.round(<number>item.reappointRate)
                    );
                    chartLabels.push(item.clinicName);
                }
            )
...
export const selectFdReappointRateTrendChartData = createSelector(
    ...
    (resBody, clinicId, trendMode) => {
        ...
        if(typeof clinicId === 'string'){
            ...
            chartDatasets = [{data: chartData, label: '', backgroundColor: DoughnutChartColors[0] }];
        }else{
            const chartData = [], targetData = [];
            resBody.data.forEach(
                item => {
                ...
                resBody.data.forEach(
                    item => {
                        if(item.clinicId){
                            chartLabels.push(item.clinicName);
                            chartData.push(
                                Math.round(<number>item.numTicks)
                            )
                        }
                    }
                );
