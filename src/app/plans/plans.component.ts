...
export class PlansComponent implements AfterViewInit {
  ...
  openDialog(): void {
    const dialogRef = this.dialog.open(DialogOverviewExampleDialogComponent, {
      ...
      data: {
        plan: this.plan,
        allowedClinics: this.allowedClinics,
  ...
  private deletePlan(row) {
    if (confirm('Are you sure to delete this plan?')) {
      if (this.rows[row]['id']) {
        this.plansService.deletePlan(this.rows[row]['id']).subscribe(
          res => {
            if (res.status == 200) {
              alert('Plan Removed');
              this.getPlans();
  ...
  updatePlan(row): void {
    const dialogRef = this.dialog.open(DialogOverviewExampleDialogComponent, {
      width: '750px',
      data: {
        plan: this.rows[row]['plan'],
        allowedClinics: this.rows[row]['allowedClinics'],
        description: this.rows[row]['description'],
        amount: this.rows[row]['amount'],
        discount: this.rows[row]['discount'],
      },
    });
    dialogRef.afterClosed().subscribe(result => {
      if (result != undefined) {
