...
export class SetupComponent implements AfterViewInit {
  ...
  getConnectCoreLink() {
    // this.clinic_id = 186;
    this.setupService.getConnectCoreLink(this.clinic_id).subscribe(
      res => {
        if (res.status == 200) {
  ...
  getMyobLink() {
    this.setupService.getMyobLink(this.clinic_id).subscribe(
      res => {
        if (res.status == 200) {
  ...
  public disconnectMyob() {
    this.setupService.clearSessionMyob(this.clinic_id).subscribe(
      res => {
        if (res.status == 200) {
          this.myobConnect = false;
          this.myobOrganization = '';
...
*/
  ...
  checkUserEmail(display_name, email, user_type) {
    this.rolesUsersService.checkUserEmail(email).subscribe(
      res => {
        if (res.status == 200) {
          let length = 10;
          var randomPassword = '';
  ...
  updateStepperStatus() {
    this.setupService.updateStepperStatus().subscribe(
      res => {
        $('.ajax-loader').hide();
        if (res.status == 200) {
          if (this.stepVal < 4) {
  ...
  public coreSyncStatus() {
    this.setupService.checkCoreSyncStatus(this.clinic_id).subscribe(
      res => {
        if (res.status == 200) {
          if (res.body.data == 1) {
            this.stepVal = 3;
  ...
  checkRepotrs() {
    var selfO = this;
    selfO.setupService.checkReportsStatus(selfO.clinic_id).subscribe(
      res => {
  ...
  checkExactRepotrs() {
    var selfO = this;
    selfO.setupService.checkExactRepotrStatus(selfO.clinic_id).subscribe(
      res => {
        let urlActive = this._location.path();
        this.reportsStatusInfo = true;
        selfO.reportsStatus = [];
