...
export class ClinicianProceeduresComponent implements AfterViewInit {
 ...
 this.route.params.subscribe(params => {
    this.clinic_id = this.route.snapshot.paramMap.get("id");
        ...
        $('#title').html('Clinician Procedures & Referrals');
...
this.preoceedureChartColors = [
 ...
 private loadDentist(newValue) {  
  if(newValue == 'all') {
    ...
    this.changePieReferral('Internal');
    ...
    this.predictedChartLabels1 =[];
       if(data.message == 'success'){
        data.data.forEach(res => {
           ...
           this.predictedPreviousTotal1 = this.predictedPreviousTotal1 + parseInt(res.trend_ratio1);

           this.predictedTotal2 = this.predictedTotal2 + parseInt(res.ratio2);
           this.predictedPreviousTotal2 = this.predictedPreviousTotal2 + parseInt(res.trend_ratio2);

           this.predictedTotal3 = this.predictedTotal3 + parseInt(res.ratio3);
           this.predictedPreviousTotal3 = this.predictedPreviousTotal3 + parseInt(res.trend_ratio3);
       ...
       this.predictedPreviousAverage1 = this.predictedPreviousTotal1/this.predictedChartData1.length;
       this.predictedPreviousAverage2 = this.predictedPreviousTotal2/this.predictedChartData2.length;
       this.predictedPreviousAverage3 = this.predictedPreviousTotal3/this.predictedChartData3.length;
...
//Total Revenue of Clinician Per Procedure
  ...
  filterDate(duration) {
    ...
    if(duration == 'w') {
      const now = new Date();
       ...
       this.duration='w';
        this.loadDentist($('.external_dentist').val());
   ...
   this.duration='m';
    ...
    $('.filter_'+duration).addClass("active");
     ...
     console.log(this.startDate+ " " +this.endDate+ " "+$('.external_dentist').val());
  ...
  changeDentistPredictor(val){
    $('.predictor_ratio .sa_tab_btn').removeClass('active');
