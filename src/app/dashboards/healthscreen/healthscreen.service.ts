...
export class HealthScreenService {
 public token: string;
   ...
   hourlyRateChart( clinic_id, startDate = '', endDate = '', duration='', user_type='',clinician='', token = this._cookieService.get("token"),limit=5 ): Observable<any> {
        ...
        return this.http.get(this.apiUrl +"/health/chHourlyLeaders?clinic_id="+clinic_id+"&start_date="+startDate+"&end_date="+endDate+"&duration="+duration+"&limit=5", { withCredentials: true})
